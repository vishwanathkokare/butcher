...
const ProductManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProductPrices = async () => {
      try {
        const response = await axios.get(`${import.meta.env.API_URL}/market/prices`);
  ...
  const handleUpdatePrice = async () => {
    if (selectedProduct && newPrice !== '') {
      try {
        const response = await axios.put(`${import.meta.env.API_URL}/market/prices/${selectedProduct._id}`, { price: newPrice });
